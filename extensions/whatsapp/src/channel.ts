...
export const createWhatsAppPlugin = (
  ...
  agentTools: () => [runtime.channel.whatsapp.createLoginTool()],
  pairing: {
  ...
  config: {
    ...
    isConfigured: async (account) => await runtime.channel.whatsapp.webAuthExists(account.authDir),
  ...
  directory: {
    self: async ({ cfg, accountId }) => {
      ...
      const { e164, jid } = runtime.channel.whatsapp.readWebSelfId(account.authDir);
      const id = e164 ?? jid;
      if (!id) {
        return null;
      }
  ...
  outbound: {
    ...
    chunker: (text, limit) => runtime.channel.text.chunkText(text, limit),
    chunkerMode: "text",
    textChunkLimit: 4000,
    pollMaxOptions: 12,
    ...
    sendText: async ({ to, text, accountId, deps, gifPlayback }) => {
      const send = deps?.sendWhatsApp ?? runtime.channel.whatsapp.sendMessageWhatsApp;
    ...
    sendPoll: async ({ to, poll, accountId }) =>
      await runtime.channel.whatsapp.sendPollWhatsApp(to, poll, {
        verbose: runtime.logging.shouldLogVerbose(),
        accountId: accountId ?? undefined,
      }),
  },
  auth: {
    login: async ({ cfg, accountId, runtime, verbose }) => {
      ...
      await runtime.channel.whatsapp.loginWeb(
        Boolean(verbose),
        undefined,
        runtime,
        resolvedAccountId,
  ...
  heartbeat: {
    checkReady: async ({ cfg, accountId, deps }) => {
      ...
      const account = resolveWhatsAppAccount({ cfg, accountId });
      const authExists = await (deps?.webAuthExists ?? runtime.channel.whatsapp.webAuthExists)(
        account.authDir,
      );
      if (!authExists) {
      ...
      const listenerActive = deps?.hasActiveWebListener
        ...
        : Boolean(runtime.channel.whatsapp.getActiveWebListener());
      if (!listenerActive) {
  ...
  status: {
    ...
    buildChannelSummary: async ({ account, snapshot }) => {
      ...
      const linked =
        typeof snapshot.linked === "boolean"
          ? snapshot.linked
          : authDir
            ? await runtime.channel.whatsapp.webAuthExists(authDir)
            : false;
      const authAgeMs =
        linked && authDir ? runtime.channel.whatsapp.getWebAuthAgeMs(authDir) : null;
      const self =
        linked && authDir
          ? runtime.channel.whatsapp.readWebSelfId(authDir)
          : { e164: null, jid: null };
      return {
        configured: linked,
        linked,
    ...
    buildAccountSnapshot: async ({ account, runtime }) => {
      const linked = await runtime.channel.whatsapp.webAuthExists(account.authDir);
      return {
        accountId: account.accountId,
        name: account.name,
        enabled: account.enabled,
    ...
    logSelfId: ({ account, runtime, includeChannelPrefix }) => {
      runtime.channel.whatsapp.logWebSelfId(account.authDir, runtime, includeChannelPrefix);
    },
  },
  gateway: {
    startAccount: async (ctx) => {
      ...
      return runtime.channel.whatsapp.monitorWebChannel(
        runtime.logging.shouldLogVerbose(),
        undefined,
        true,
        undefined,
        ctx.runtime,
    ...
    loginWithQrStart: async ({ accountId, force, timeoutMs, verbose }) =>
      await runtime.channel.whatsapp.startWebLoginWithQr({
        accountId,
        force,
        timeoutMs,
        verbose,
    ...
    loginWithQrWait: async ({ accountId, timeoutMs }) =>
      await runtime.channel.whatsapp.waitForWebLogin({ accountId, timeoutMs }),
    logoutAccount: async ({ account, runtime }) => {
      const cleared = await runtime.channel.whatsapp.logoutWeb({
